...
def _mint_with_browser_device_code(
    *,
    email: str,
    password: str,
    page: Any | None = None,
...
def _mint_with_browser_pkce(
    *,
    ...
    """Authorize Grok Build with authorization-code + PKCE.

    This is the flow that issues access tokens with referrer=grok-build.
    """
    from .oauth_pkce import exchange_authorization_code, create_pkce_session
    from .proxyutil import proxy_log_label, resolve_proxy, set_runtime_proxy
    ...
    try:
        ...
        log(
            f"pkce auth prepared redirect={session.redirect_uri} "
            f"proxy={proxy_log_label(resolved) or '(none)'} referrer=grok-build"
        )
        ...
        approve_device_code(
            work_page,
            verification_uri_complete=session.authorization_url,
            email=email,
            password=password,
            user_code="",
            timeout_sec=browser_timeout_sec,
            stop_event=session.server.done,
            log=log,
        )
        ...
        code = session.wait_code(timeout=max(browser_timeout_sec, 60.0))
        log("pkce callback received; exchanging authorization code")
        tr = exchange_authorization_code(
            session,
            code,
            proxy=resolved or None,
            timeout=30.0,
        ...
        success = True
        log("pkce token exchange SUCCESS")
        return {
            "access_token": tr.access_token,
            "refresh_token": tr.refresh_token,
            "id_token": tr.id_token,
...
def mint_with_browser(
    *,
    ...
    flow = (os.environ.get("CPA_OAUTH_FLOW") or "pkce").strip().lower()
    kwargs = dict(
        email=email,
        password=password,
        page=page,
    ...
    if flow in {"device", "device-code", "device_code"}:
        return _mint_with_browser_device_code(**kwargs)
    return _mint_with_browser_pkce(**kwargs)
